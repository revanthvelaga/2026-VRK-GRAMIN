(()=>{
 ...
 function accountNav(){
  ...
  const ownerTools=localUser.role==='admin'?`<label class="owner-tools">Owner tools<select id="owner-tools"><option value="">Choose an action</option><option value="tickets">Customer tickets</option><option value="assign">Assign technician</option><option value="technicians">Technicians (${data?.technicians?.length||0})</option><option value="monitor">Technician dashboard</option><option value="branches">Add / edit branches</option><option value="admins">Add new Admin</option><option value="settings">Service settings</option></select></label>`:'';
