(()=>{
 ...
 function accountNav(){
  ...
  nav.innerHTML=`<div class="account-nav">${ownerSwitch}<span class="account-name"><strong>${esc(localUser.name||localUser.login)}</strong><small>${esc(localUser.login)}</small></span>${role==='customer'?'<button class="quiet" id="tickets-nav">My Tickets</button>':''}<button class="quiet" id="logout">Log out</button></div>`;
