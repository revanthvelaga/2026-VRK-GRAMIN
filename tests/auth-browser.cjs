...
(async()=>{
 ...
 await bookingForm.locator('[name="phone"]').fill('9876543210');
 await bookingForm.locator('[name="village"]').fill('G.Koduru');
 await bookingForm.locator('[name="pin"]').fill('531113');
 await bookingForm.locator('[name="landmark"]').fill('Temple road');
 await bookingForm.locator('[name="addressLabel"]').fill('Parents home');
 await bookingForm.locator('[name="saveAddress"]').check();
 ...
 await page.locator('[data-service="plumbing"]').click();
 await page.locator('#saved-address').waitFor();
 assert.equal(await page.locator('#saved-address option').first().textContent(),'Add a new address');
 assert.match(await page.locator('#saved-address option').nth(1).textContent(),/Parents home/);
 await page.locator('#saved-address').selectOption({index:1});
 assert.equal(await page.locator('#booking-form [name="village"]').inputValue(),'G.Koduru');
 await page.locator('#saved-address').selectOption('');
 assert.equal(await page.locator('#booking-form [name="village"]').inputValue(),'');
 assert.equal(await page.locator('#booking-form [name="saveAddress"]').isChecked(),true);
 await page.locator('#back').click();
 ...
 console.log('PASS: browser login, service-only address capture and reuse, booking to Admin queue, technician onboarding/approval, Owner-only Admin creation, direct role routing, and mobile layout.');
 await browser.close();
