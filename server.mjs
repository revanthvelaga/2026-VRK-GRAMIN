...
const publicUser=account=>({id:account.role==='technician'&&account.technicianId?account.technicianId:account.id,accountId:account.id,login:account.login,name:account.name,role:account.role,status:account.status||'active',technicianId:account.technicianId||null,profile:account.profile||{}});
...
http.createServer(async(req,res)=>{try{
 ...
 if(url.pathname.startsWith('/api/')){
  ...
  if(req.method==='POST'&&url.pathname==='/api/actions'){
   ...
   const parsed=await readBody(req);const result=await transact(next=>apply(next,user,parsed.action,parsed.input,req.headers['idempotency-key']));return send(res,result);
  }
  fail('API route not found',404);
 }
 if(!['GET','HEAD'].includes(req.method)){res.writeHead(405);return res.end();}
 const file=path.resolve(root,'.'+decodeURIComponent(url.pathname==='/'?'/index.html':url.pathname));if(!file.startsWith(root+path.sep)){res.writeHead(403);return res.end();}let data=await readFile(file);if(path.basename(file)==='index.html')data=Buffer.concat([data,Buffer.from('<script src="/local-auth.js"></script>')]);res.setHeader('Content-Type',({'html':'text/html; charset=utf-8','css':'text/css; charset=utf-8','js':'text/javascript; charset=utf-8'})[file.split('.').pop()]||'application/octet-stream');res.end(req.method==='HEAD'?undefined:data);
